...
const Stories = (
  ...
  const mutation = api.postReadlist.delete.useMutation();

  const handleRemovePostFromReadlist = (postId: string) => {
    mutation.mutate(
      {
        postId: postId,
      ...
      },
      {
        ...
        onError: () => {
          if (mutation.error?.message)
            toast.error(`Removed failed: ${mutation.error.message}`);
        },
      }
    );
  };
  ...
  return (
    <MainAccount>
      <h1 className="text-xl font-medium text-textNavbar md:mt-5 md:text-3xl">
        {props.readlist.name}
        <span className="ml-5 text-sm text-textBio md:text-base">
          {` ${stories.length}  ${
                ...
                >
                  ...
                  <div className="flex justify-between">
                    ...
                    <button
                      ...
                      onClick={() => handleRemovePostFromReadlist(story.id)}
                    >
                      <svg
